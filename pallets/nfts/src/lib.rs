...
use self::nfts::{NFTData, NFTId, NFTSeriesDetails, NFTSeriesId};
use frame_support::{pallet_prelude::ensure, traits::StorageVersion};
...
pub mod nfts {
	...
	impl<AccountId> NFTData<AccountId> {
		pub fn new(
			...
			Self { owner, ipfs_reference, series_id, locked }
		}
	}
...
pub mod pallet {
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn create(
			...
			let value = NFTData::new(who.clone(), ipfs_reference.clone(), series_id.clone(), false);
